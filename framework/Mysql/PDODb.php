...
class PDODb
{
    ...
    public function connect()
    {
        ...
        $connectionParams = array('host', 'dbname', 'port', 'charset');
    ...
    {
        $this->query = 'SELECT count(*) total FROM ' . $this->getTableName($tableName);
        $stmt        = $this->buildQuery(null);
        $stmt->execute();
        $this->lastError     = $stmt->errorInfo();
        $this->lastErrorCode = $stmt->errorCode();
        $this->rowCount      = $stmt->rowCount();
    ...
    public function setQueryOption($options)
    {
        $allowedOptions = array('ALL', 'DISTINCT', 'DISTINCTROW', 'HIGH_PRIORITY', 'STRAIGHT_JOIN', 'SQL_SMALL_RESULT',
            ...
            'LOW_PRIORITY', 'IGNORE', 'QUICK', 'MYSQLI_NESTJOIN', 'FOR UPDATE', 'LOCK IN SHARE MODE', 'COUNT(*)');
        if (!is_array($options)) {
    ...
    public function startTransaction()
    {
        ...
        register_shutdown_function(array($this, "checkTransactionStatus"));
    }
    /**
    ...
    {
        return new self(array('type' => $this->connectionParams['type'], 'host' => $subQueryAlias, 'isSubQuery' => true, 'prefix' => $this->prefix));
    }
    /**
