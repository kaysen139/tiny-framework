...
class Connection
{
     ...
     */
    public function execute($namespace, BulkWrite $bulk, WriteConcern $writeConcern = null, $ignore = false)
    {
     ...
     * @return integer
     */
    public function insertAll(Query $query, array $dataSet, $ignore = false)
    {
        // 分析查询表达式
        $options = $query->getOptions();
        ...
        // 生成bulkWrite对象
        $bulk         = $this->builder->insertAll($query, $dataSet, $ignore);
        $writeConcern = isset($options['writeConcern']) ? $options['writeConcern'] : null;
        $writeResult  = $this->execute($options['table'], $bulk, $writeConcern, $ignore);
